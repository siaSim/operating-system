...
{                                            // 스레드 코드
  pthread_mutex_timedlock(&lock1, &timeout); // x를 독점 사용하기 위해 lock1 잠그기
  printf("%s lock1 잠금\n", (char *)arg);
  x++;
  sleep(2); // 2초 잠자기

  pthread_mutex_timedlock(&lock2, &timeout); // y를 독점 사용하기 위해 lock2 잠그기
  printf("%s lock2 잠금\n", (char *)arg);
  y++;
...
int main()
{
  ...
  timeout.tv_sec = time(NULL) + 10; // 10초후
  timeout.tv_nsec = 0;
