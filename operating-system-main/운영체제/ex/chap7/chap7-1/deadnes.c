...
{                             // 스레드 코드
  ...
  pthread_mutex_lock(&lock1); // y를 독점 사용하기 위해 lock2 잠그기
  printf("%s lock1 잠금\n", (char *)arg);
  y++;
  // pthread_mutex_unlock(&lock1); // lock2 풀기
  // printf("%s lock1 해제\n", (char *)arg);
...
{                             // 스레드 코드
  ...
  pthread_mutex_lock(&lock2); // x를 독점 사용하기 위해 lock1 잠그기
  printf("%s lock2 잠금\n", (char *)arg);
  x++;
...
int main()
{
  ...
  pthread_mutex_init(&lock1, &mutexattr); // 뮤텍스 락 변수 lock1 초기화
  pthread_mutex_init(&lock2, &mutexattr); // 뮤텍스 락 변수 lock2 초기화
